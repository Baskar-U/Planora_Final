// Browser-based script to add demo packages
...
// Function to add demo packages
async function addDemoPackages() {
  try {
    console.log('🚀 Starting to add demo packages for all vendors...');
    
    // Import Firebase functions
    const { getFirestore, collection, getDocs, doc, updateDoc, serverTimestamp } = await import('https://www.gstatic.com/firebasejs/10.7.1/firebase-firestore.js');
    ...
    for (const docSnapshot of snapshot.docs) {
      ...
      const vendorRef = doc(db, 'postorder', vendorId);
...
1. Open your browser and go to your Planora application
2. Open Developer Tools (F12)
3. Go to the Console tab
4. Copy and paste this entire script
5. Press Enter to run it

⚠️  Make sure you are logged in as an admin user before running this script.

The script will:
- Keep existing packages for: Tuto Decorations, Cookies Photography, Tutoapp, Tuto Travels
- Replace packages for all other vendors with 2-3 demo packages based on their category
- Show progress in the console
`);

// Export the function for manual execution
window.addDemoPackages = addDemoPackages;
